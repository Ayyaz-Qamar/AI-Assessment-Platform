...
from app.ai.badges_engine import compute_user_badges, newly_earned_badges
...
def _finalize_attempt(db: Session, attempt: TestAttempt, user_id: int) -> NextQuestionResponse:
    # Snapshot badges BEFORE finalizing this attempt (so we can diff afterwards)
    _, before_map = compute_user_badges(db, user_id)
